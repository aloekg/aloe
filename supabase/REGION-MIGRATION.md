...
- **Staging.** Проект из `.env.local` (`puqmkruyhjfvhyjfriig`) никуда не переезжал и остался в своём
  регионе — ему лишь переписали URL картинок на новый прод (см. «Как прошло»). Если его понадобится
  пересоздать, в том числе чтобы сменить регион, — это отдельный и куда более дешёвый порядок
  действий: раздел «Стейдж» в конце файла.
