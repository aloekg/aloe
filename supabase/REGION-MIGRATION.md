...
- **Staging.** Старый проект (`puqmkruyhjfvhyjfriig`, ap-northeast-1, переименован в
  `aloe-staging-old`) вместе с продом не переезжал — ему лишь переписали URL картинок на новый прод
  (см. «Как прошло»). Пересоздан отдельно и по другому, куда более дешёвому порядку действий —
  раздел «Стейдж» в конце файла.
...
Стейджу понадобился тот же `supabase/sql/rewrite-storage-urls.sql`: его строки указывали на
продовые бакеты Мумбая (так задумано, см. CODEBASE.md → Environments), и с паузой старого проекта
картинки там стали битыми. После переписывания — ноль ссылок на старый хост в `products`,
`categories`, `banners` и в замороженных `orders.items`.
...
**Выполнено 21 сентября 2026**, в тот же день, что и прод.

|        | старый                       | новый                    |
| ------ | ---------------------------- | ------------------------ |
| ref    | `puqmkruyhjfvhyjfriig`       | `zzyeeazpxgpfenfyxqed`   |
| регион | ap-northeast-1 (Токио)       | eu-central-1 (Франкфурт) |
| имя    | `aloe-staging-old`, на паузе | `aloe-staging`           |
...
1. **Проверить лимит проектов в организации.** На бесплатном плане активных два, и старый стейдж
